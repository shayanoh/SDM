...
**Status: IMPLEMENTED — merged to `main` 2026-09-02.** This plan is a historical record. It has no open work: anything its Self-Review deferred "to a later part" was either completed in Parts 4–5 or reviewed and moved to `todo.md` at the repo root. In particular, `ItemState.assembling` and per-component connection-demand precision were both reviewed and **kept as-is** (see `todo.md` → "Decided — no change"). Do not treat the "Deferred" notes below as a backlog.
